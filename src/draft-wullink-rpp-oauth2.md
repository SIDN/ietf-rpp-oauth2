...
A key design goal of RPP's authorization model is fine-grained access control. A registrar MUST be able to operate multiple user accounts within the registry, each carrying a distinct set of permissions appropriate to the user's or tool's role (e.g., read-only reporting accounts, accounts limited to a specific set of operations, or fully privileged administrative accounts). This allows registrars to implement the principle of least privilege within their own organizations without requiring separate registry-level registrar accounts.

A> PK> Removal of "The registry's AS MUST support registrar self-service management of these user accounts, enabling registrars to create, modify, and revoke user credentials and their associated permission scopes without requiring manual intervention by the registry operator." - because - how the management of the users works is not for the protocol to define. It may be out-of-bacnd and the protocol will/shall work exactly the same.
On a side note: normative MUST "A registrar MUST be able to operate multiple user accounts within the registry" is on the edge of being an architectural statement or design goal and is even policy setting. I don't think it belongs to inroduciton.
...
A> PK> is normative "MUST include" actually belonging to this draft? It is defined in the core spec in fact.
...
A> PK> I am always a bit sceptical about normative requirements put into definitions.
...
A> PK> I don't think we need OIDC
...
A> PK> what are sensitive claims? How would an issuer of the token know if the token would be persisted?
...
A> PK> Term "Policy Enforcement Point" is not defined. The term is actually normatively defined in xacml (https://docs.oasis-open.org/xacml/3.0/xacml-3.0-core-spec-os-en.html#_Toc325047169), but likely this is not what is meant here. Also "RPP operates" reads wrong. The protocol cannot operate anything. Likely "RPP Server operates".
A> I think the key exchange can also happen out-of-band
...
OAuth scopes are used for granting authorisation and enforcing access control when accessing RPP resources. The server MUST define a set of scopes that can be requested by clients when obtaining access tokens. The scopes MUST be based on the principle of least privilege, allowing clients to request only the permissions they need to perform their intended operations. The server MUST also define the mapping between scopes and the specific resources and operations that they grant access to.

PK> "The scopes MUST be based on the principle of least privilege, allowing clients to request only the permissions they need to perform their intended operations." - is it a normative requirement to the protocol itself (like architectural statement), or what is the role of this sentence?
...
A> PK> This paragraph reads like repeating OAuth with no own meaning.
...
A> PK> I think the scopes shall have some namespace structure, so that AS can be used also for other, even not RPP related services without conflicts.
Something like `rpp:<server>:<object>:<access-level>`. `<server>` shall allow to cover for more than one RPP server instance behind one AS.
...
A> PK> shall we include "catch all" permissions or wildcards - domain:* or *:read or finally *:* ?
...
A> PK> if additional specification is always needed alongside of data objects specification that would mean that also data objects itself would need to define own scopes. When you ask me it is not needed at all, because the derivation rules apply same all the time. So the Scope Registry below can be even considered non-normative.
...
A> PK> I think talking about registrar's AS is likely the other draft. 

When the authorizing registrar's AS does not support RAR ([@!RFC9396]) .... **TODO**

A> PK> this is not finished.
...
A> PK> "RPP access tokens MUST conform to the JWT Profile for OAuth 2.0 Access Tokens defined in [@!RFC9068]." - this is stated normatively 3rd time already. Strike it.
...
A> PK> I think it is fine to list them if:
...
A> PK> I think talking about registrar's AS is likely the other draft. If it is the same draft than it should be a separate chapter with all the aspect of security and trust (which I do not yet comprehend fully), and not hidden in "Claims" section.
A> PK> this is somehow tightly bound to specific business setups of R-R-R model. In R-R model it may be different.
Also, this definition of "sub" seems to be already repeating content from JWT Profile Claims section.
...
A> PK> Something for operational considerations?
...
A> PK> I think there is an IANA registry for JWT claims

## Claim Validation

The RPP server MUST validate all required claims in accordance with [@!RFC9068] and [@!RFC8725].

A> PK> I would just refer to the normative RFCs and not repeat 
...
A> PK> I think we should include client as a general concept in Data Objects, but same time extend it here with OAuth-specific attributes.

# Managing Trust

A> PK> the title of the chapter does not seem to match the content
...
A> PK> is this a conditional MUST (if advanced use case), or a generally applicable MUST?
...
A> PK> this recommendation is also all over the place. Reduce repeats.
...
A> PK> not sure what to think of it, but as this is optional let's leave it for now.
...
A> write something about PII of sub if this is user identifier
